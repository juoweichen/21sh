...
void	update_pwd(t_exec *exec, char *next_dir)
{
	...
	if ((pwd = dict_get(exec->env_dict, "PWD")) == NULL ||
		(oldpwd = dict_get(exec->env_dict, "OLDPWD")) == NULL ||
		next_dir == NULL)
		return ;
	dict_add(exec->env_dict, "OLDPWD", pwd, ft_strlen(pwd));
	dict_add(exec->env_dict, "PWD", next_dir, ft_strlen(next_dir));
}
...
void	cd_builtin(t_exec *exec, char **arg)
{
	...
	{
		if (ft_strequ(arg[1], "-") &&
			(com_path = dict_get(exec->env_dict, "OLDPWD")) != NULL)
			next_dir = ft_strdup(com_path);
		else if (arg[1][0] == '/')
